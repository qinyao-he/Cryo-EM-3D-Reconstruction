...
class UnknownRSKernel:
    ...
    def set_proj_quad(self, rad):
        ...
        interp_params_RI = {'N': self.N, 'kern': kern_R, 'kernsize': kernsize_R, 'rad': rad, 'zeropad': zeropad_R,
                            'dopremult': dopremult_R}
        ...
        if domain_change_RI or interp_change_RI:
            ...
            print "  Projection Ops: %d (%d slice, %d inplane), " % (self.N_RI, self.N_R, self.N_I), ;
            sys.stdout.flush()
    ...
    def set_slice_quad(self, rad):
        ...
        slice_params = {'quad_type': quad_scheme_R, 'degree': degree_R,
                        'sym': sym}
        interp_params_R = {'N': self.N, 'kern': kern_R, 'kernsize': kernsize_R, 'rad': rad, 'zeropad': zeropad_R,
                           'dopremult': dopremult_R}
    ...
    def set_inplane_quad(self, rad):
        ...
        inplane_params = {'degree': degree_I}
        interp_params_I = {'N': self.N, 'kern': kern_I, 'kernsize': kernsize_I, 'rad': rad, 'zeropad': zeropad_I,
                           'dopremult': dopremult_I}
    ...
    def set_shift_quad(self, rad):
        ...
        if domain_change or interp_change:
            print "  Shift Ops: %d, generated in" % self.N_S, ;
            sys.stdout.flush()
    ...
    def set_data(self, cparams, minibatch):
        ...
        if interp_change:
            print "Iteration {0}: freq = {3}, rad = {1}, N_T = {2}".format(cparams['iteration'], rad, self.N_T,
                                                                           max_freq)
            self.rad = rad
        ...
        _, _, fspace_truncmask = gencoords(self.fspace_stack.get_num_pixels(), 2,
    ...
    def get_result_struct(self):
        ...
        res['kern_timing'] = {'prep_sample_R': n.empty(N_M), 'prep_sample_I': n.empty(N_M),
                              'prep_sample_S': n.empty(N_M),
    ...
    def prep_operators(self, fM, idx, slicing=True, res=None):
        ...
        if slicing:
            if not factoredRI:
                ...
                W_R_sampled = (W_R_sampled.reshape((N_R_sampled, 1)) * W_I_sampled.reshape((1, N_I_sampled))).reshape(
                    (N_R_sampled * N_I_sampled,))
