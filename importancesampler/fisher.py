...
pyximport.install(setup_args={"include_dirs": n.get_include()}, reload_support=True)
import fisher_util
...
def getkernmat(dirsout, dirsin, kappa, eps, chiral_flip):
    ...
    if arghash in precomputed_kernmats:
        ...
        kdout = ss.cKDTree(dirsout * (
            1.0 + eps))  # this is to make sure the distance between identicals is nonzero (bad hack but it works)
