...
def find_L(x, f, dfdx, evalobj, L, maxIts=None, \
    ...
    while maxIts == None or its < maxIts:
        ...
        eps = gnorm / currL
        if ostream != None:
            ostream('{its}: L = {0}, abs cond = {1}, rel cond = {2}, predL = {3}'.format(currL, condVal, relCondVal,
                                                                                         predFitL, its=its))
    ...
    if ostream != None:
        ostream("Found {4} L = {0} (f - fp = {2}, 0.5*gnorm2/L = {3}) after {1} its".format(finalL, its, f - finalfp,
                                                                                            0.5 * gnorm2 / finalL,
                                                                                            solnType))
    return finalL
...
class SAGDStep(base.BaseStep):
    ...
    def do_step(self, x, params, cryodata, evalobj, batch, **kwargs):
        ...
        g2_lambdaw = params.get('sagd_lambdaw',
                                10)  # weight of the prior which biases the covariance estimate towards lambda
