...
def param2density(x, xtype, sz, precond=None):
    if xtype == 'real':
        ...
        fM[0:NC, :, :] = herm_freqs
        if startFreq:
            fM[NC:, :, :] = n.roll(n.roll(herm_freqs[startFreq:zeroFreq, :, :][::-1, ::-1, ::-1].conj(), 1, axis=1), 1,
                                   axis=2)
        else:
...
class CryoOptimizer(BackgroundWorker):
    ...
    def __init__(self, expbase, cmdparams=None):
        ...
        if init_model is not None:
            ...
            print "Randomly generating initial density (init_random_seed = {0})...".format(init_seed),
            sys.stdout.flush()
            tic = time.time()
        ...
        tic = time.time()
        print "Windowing and aligning initial density...",
        sys.stdout.flush()
        ...
        if modelscale == 'auto':
            ...
            print "estimated modelscale = {0:.3g}...".format(modelscale),
            sys.stdout.flush()
    ...
    def eval_params(self):
        ...
        if 'name' not in self.cparams:
            self.cparams['name'] = '{0} - {1} - {2} ({3})'.format(self.cparams['dataset_name'],
                                                                  self.cparams['prior_name'],
                                                                  self.cparams['optimizer_name'],
                                                                  self.cparams['kernel'])

    def run(self):
        while self.dowork(): pass
        print "Waiting for IO queue to clear...",
        sys.stdout.flush()
        self.io_queue.join()
        print "done."
        sys.stdout.flush()
    ...
    def dowork(self):
        ...
        apply_sym = sym is not None and self.cparams.get('perfect_symmetry', True) and self.cparams.get(
            'apply_symmetry', True)
        if apply_sym:
        ...
        if save_it:
            ...
            if self.io_queue.qsize():
                print "Warning: IO queue has become backlogged with {0} remaining, waiting for it to clear".format(
                    self.io_queue.qsize())
                self.io_queue.join()
            ...
            if checkpoint_it:
                ...
                self.io_queue.put(('cp', opj(self.outbase, 'model.mrc'),
                                   opj(self.outbase, 'model-{0:06}.mrc'.format(self.iteration))))
